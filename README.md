...
The API validates HTTP requests, dispatches messages, and returns acceptance
receipts.
Session logic owns agent and model behavior and can use the native Linux
runtime. An App deploys one Session logic, which governs every Session opened
for that App.
...
export default defineApi<Input>(({ app, router }) => {
  ...
  router.route("POST", "/dispatch", async ({ request }) => {
    ...
    const receipt = await session.dispatch(body.input);
...
creates the logical Session if its ID does not exist and enqueues the message;
...
request. Native Session logic receives the same values as a read-only
`SessionIdentity`.
...
the active Session runtime accepts it into its in-memory mailbox:
...
router.route("POST", "/github", async ({ request }) => {
  ...
  return Response.json({ accepted: true, messageId: receipt.id }, {
    status: 202,
  });
});
```
...
completed result instead of invoking the Session logic again. Calling `dispatch()` a
...
receipt therefore confirms volatile acceptance, not durable persistence, and
does not contain the eventual harness output. IDs also cannot make arbitrary
...
signals, and Web Crypto. They must not import Session logic or depend on Node.js,
...
Linux VM. Each App has one deployed Session logic; callers opening a Session do
...
export default defineSessionLogic<Input, RuntimeEvent>(
...
Session logic may use Node.js, subprocesses, the Linux filesystem, provider
...
Every logic invocation receives the `SessionIdentity` represented by its Edge
...
logic for one message at a time. Session logic can use a payload discriminator and its
...
```ts
export default defineSessionLogic<Message>(async (context) => {
...
The harness is deployment infrastructure around the Session logic. Cantelop's
...
The user-defined Session logic owns message settlement. Resolving completes
...
Native Session logic can emit application-defined events. Edge Session
dispatch returns only an acceptance receipt and does not expose those events
through the API runtime.
