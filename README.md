...
Session handle without making a request. The first `dispatch()` atomically
...
native harness. `SessionHandle<Input>` extends it with Edge operations.
...
router.route("POST", "/github", async ({ request }) => {
  ...
  const receipt = await session.dispatch(event, {
    idempotencyKey: request.headers.get("X-GitHub-Delivery") ?? undefined,
  });
...
The execution ID is generated by the SDK. An application can also supply an
opaque `idempotencyKey` of at most 256 bytes and must reuse that key when it
retries the same logical message. Within one active Session, duplicate
execution IDs share the original in-flight or completed result instead of
invoking the harness again.
...
input. It does not assign meaning such as run, steer, or cancel. The active
runtime places executions in an in-memory FIFO mailbox and invokes the harness
for one message at a time. A harness can use an input discriminator and its
