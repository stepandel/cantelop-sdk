...
The execution ID is generated by the SDK. The platform must preserve that ID
when it retries delivery to the active Session runtime. Within one activation,
duplicate deliveries of the same execution ID share the original in-flight or
completed result instead of invoking the harness again. Calling `dispatch()` a
second time creates a new logical message with a new execution ID.
