...
export class RuntimeMessages {
  ...
  admit(id: string, semanticEnvelope: unknown, enqueue: (signal: AbortSignal, started: () => void) => { generation: number; settled: Promise<void> }): { receipt: MessageReceipt; settled: Promise<void> } {
    const fingerprint = createHash("sha256").update(canonical(semanticEnvelope)).digest("hex");
    ...
    if (this.records.size >= this.maxMessages) throw new RuntimeProtocolError(429, "mailbox_capacity");
    const now = Date.now();
    const controller = new AbortController();
    const receipt: MessageReceipt = { sandbox_id: this.sandboxId, message_id: id, sequence: this.records.size + 1,
      generation: 0, accepted_at: new Date(now).toISOString(), deadline: new Date(now + this.timeoutMs).toISOString(), state: "queued" };
    ...
    const timer = setTimeout(() => this.cancel(id), this.timeoutMs);
    timer.unref();
